 ...
 * 2011/02/06  : Martyn eggleton trying to make it so this plugin can be inline or block can only fore it to be inline atm
 */
...
class syntax_plugin_condition extends DokuWiki_Syntax_Plugin {
	...
	function connectTo($mode){
		...
		//$this->Lexer->addEntryPattern('<(?:if|IF)(?=.*?</(?:if|IF)>)', $mode, 'plugin_condition');
		//$this->Lexer->addEntryPattern('<IF(?=.*?</IF>)', $mode, 'plugin_condition');
	}

	function postConnect() {
	  $this->Lexer->addExitPattern('</if>', 'plugin_condition');
		//$this->Lexer->addExitPattern('</(if|IF)>', 'plugin_condition');
	}

	// Handle the match
	function handle($match, $state, $pos, &$handler) {
	  if($state != DOKU_LEXER_UNMATCHED) return false;
		//echo "\n<br><pre>\nmatch =" .var_export(htmlentities($match), TRUE)."</pre>";
	  
	  //echo "\n<br><pre>\nhandler =" .var_export($handler, TRUE)."</pre>";
		...
		//echo "\n<br><pre>\nblocks =" .var_export($blocks, TRUE)."</pre>";
	...
	function render($mode, &$renderer, $data) {
		...
		if($mode == 'xhtml') {
			...
			if(!$bug) {
			  ...
			  foreach($instr as $instruction) {
			    if(in_array($instruction[0], array('p_open','p_close')))// 'cdata' && count($instruction[1]) == 1 && $instruction[1][0] =="\n")
			    {
			      //echo ";)";
			    }
			    else
		...
		if($mode == 'metadata') {
			...
			if(is_array($metatoc) && is_array($renderer->meta['description']['tableofcontents'])) $renderer->meta['description']['tableofcontents'] =  array_merge($metatoc, $renderer->meta['description']['tableofcontents']);
